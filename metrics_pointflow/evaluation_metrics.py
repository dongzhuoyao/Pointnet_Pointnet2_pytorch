...
def metric_wrapper(sample_pcs, ref_pcs, batch_size = 4):
    result_dict = dict()

    result = compute_all_metrics(sample_pcs=sample_pcs, ref_pcs=ref_pcs, batch_size=batch_size, accelerated_cd=False)
    pprint(result)
    result_dict['1-NN-CD-acc'] = result['1-NN-CD-acc'].item()
    result_dict['1-NN-EMD-acc'] = result['1-NN-EMD-acc'].item()

    result_dict['lgan_cov-CD'] = result['lgan_cov-CD'].item()
    result_dict['lgan_cov-EMD'] = result['lgan_cov-EMD'].item()
    result_dict['lgan_mmd-CD'] = result['lgan_mmd-CD'].item()
    result_dict['lgan_mmd-EMD'] = result['lgan_mmd-EMD'].item()

    result_dict['lgan_mmd_smp-CD'] = result['lgan_mmd_smp-CD'].item()
    result_dict['lgan_mmd_smp-EMD'] = result['lgan_mmd_smp-EMD'].item()
...
if __name__ == "__main__":
    ...
    sample_pcs = torch.rand(B,N,3)
    ref_pcs = torch.rand(B,N,3)
    pprint(metric_wrapper(sample_pcs=sample_pcs, ref_pcs=ref_pcs, batch_size=4))
    batch_size = 4
